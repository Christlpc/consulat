...
export default async function ConsularDocumentDetailPage({ params }: { params: { id: string } }) {
  const application = await prisma.consularDocument.findUnique({
    where: { id: params.id },
  });
  ...
  const docTypeLabels: Record<string, string> = {
    ...
    CERTIFICAT_COUTUME: 'Certificat de Coutume',
    AUTRE: 'Autre',
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-start print:hidden">
        <div className="flex items-center gap-4">
          <Link href="/admin/demandes/actes-consulaires" className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Demande d'Acte Consulaire</h1>
      ...
      <div className="print:hidden">
        <ApplicationActions
          ...
          applicationType="consular-document"
      ...
      <div className="print:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <dl className="space-y-3">
              <div>
                <dt className="text-sm font-medium text-gray-500">Type d'acte demandé</dt>
                <dd className="mt-1 text-sm text-gray-900 font-medium">
                  {docTypeLabels[application.documentType]}
                </dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">Objet de la demande</dt>
                <dd className="mt-1 text-sm text-gray-900">{application.purpose}</dd>
              </div>
              {application.documentToLegalize && (
                <div>
                  <dt className="text-sm font-medium text-gray-500">Document à légaliser</dt>
                  <dd className="mt-1 text-sm text-gray-900">{application.documentToLegalize}</dd>
                </div>
              )}
            </dl>
          </div>
        ...
        {(application.status === 'APPROVED' || application.status === 'READY') && (
          <div className="hidden print:block mt-16 pt-8 border-t border-gray-300">
            <div className="text-center mb-8">
              <p className="font-bold text-lg mb-4">Le Consul Général de la République du Congo en Tunisie</p>
              <p>Certifie que {application.firstName} {application.lastName}</p>
              <p className="mt-4">{application.purpose}</p>
            </div>
            <div className="flex justify-between mt-12">
              <div className="text-center">
                <p className="mb-2">Fait à Tunis, le {format(new Date(), 'dd MMMM yyyy', { locale: fr })}</p>
                <p className="mb-16 mt-8">Signature du Consul Général</p>
                <div className="border-t border-black w-48 mx-auto"></div>
              </div>
              <div className="text-center">
                <p className="mb-16 mt-8">Cachet du Consulat</p>
