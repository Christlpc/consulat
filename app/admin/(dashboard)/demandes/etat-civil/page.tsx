...
export default async function CivilStatusPage() {
  const applications = await prisma.civilStatusDocument.findMany({
    orderBy: {
      submittedAt: 'desc',
    },
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Actes d'État Civil</h1>
              ...
              {applications.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center text-gray-500">
                    Aucune demande d'acte d'état civil pour le moment
                  </td>
                </tr>
              ) : (
                applications.map((application) => (
                  <tr key={application.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <Link
                        href={`/admin/demandes/etat-civil/${application.id}`}
